layui.use(['tree', 'table', 'carousel', 'util'], function(){
    ...
    showFile=function(f,t){
      ...
      if(t=="img"){
          $.ajax({
            url: "/pass/file-list?f="+fpath+"&t=img",
            method: "get",
            data: {},
            success: function(res) {
                ...
                for(var key in res.data) {
                  ...
                  str+='<div><img src="/files'+res.data[key].path+'" style="max-width:100%;max-height:95vh;" onclick="openPage(\'http://'+window.location.host+'/files'+res.data[key].path+'\')"></div>';
                }
                str+="</div></div>";
                $("#content").html(str);
                carousel.render({
